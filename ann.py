...
def generate_model(num_inputs, num_outputs, num_hidden_layers, activation_function):
    ...
    for i in range(0, num_hidden_layers):
        nodes = int(np.interp(i, [0, num_hidden_layers], [num_inputs, num_outputs]))
...
def train_network (ciphers, activation_function, dense_layers, **kwargs):
    ...
    # Create model
    print("Generating model with {0} features, {1} outputs, {2} dense layers, and activation function '{3}'..".format(n_features, n_outputs, dense_layers, activation_function))
    model = generate_model(n_features, n_outputs, dense_layers, activation_function)
    model.summary()

    # Compile
    print("Compiling model with {0} features, {1} outputs, {2} dense layers, and activation function '{3}'..".format(n_features, n_outputs, dense_layers, activation_function))
    model.compile(optimizer='adam', loss=loss_fn, 
        metrics=['accuracy',F1])

    # Train
    print("Training model with {0} features, {1} outputs, {2} dense layers, and activation function '{3}'..".format(n_features, n_outputs, dense_layers, activation_function))
    training = model.fit(x=train_data, y=train_labels, **kwargs)
    return (model, training)

def train_layers(ciphers, function, dense_layers_range, **kwargs):
    train_time = []
    for dense_layers in dense_layers_range:

        os.makedirs('anns/{0}/'.format(function), exist_ok=True)
        os.makedirs('plots/ann/optimization/{0}/'.format(function), exist_ok=True)

        sw = Stopwatch(True)
        model, training = train_network(ciphers, function, dense_layers, **kwargs)
        model.save('anns/{0}/{1}_layer.h5'.format(function, dense_layers))
        time = sw.end()
        ...
        plt.savefig('plots/ann/optimization/{0}/{1}_layer.png'.format(function, dense_layers))
...
for function in all_train_time:
    plt.plot(list(map(lambda x: str(x), dense_layer_range)), all_train_time[function], label=function)

plt.legend()
plt.savefig('plots/ann/optimization/computation_time.png')